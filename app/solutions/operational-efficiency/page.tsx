        ...
        <div className="max-w-5xl mx-auto px-6 py-20 md:py-28 relative">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {savingsBreakdown.map((row) => (
              <div key={row.category} className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 flex items-start gap-4 hover:border-amber/30 transition-all">
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold text-green-400 mt-0.5">{row.annual}<span className="text-sm font-normal text-warm-500">/year</span></div>
                  <div className="text-sm text-warm-500 mt-1">{row.detail}</div>
                </div>
              </div>
            ))}
          </div>

          <div className="mt-8 rounded-2xl border border-amber/20 bg-amber/5 p-6 flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="text-sm font-semibold text-warm-400 uppercase tracking-wider">Total estimated savings</div>
              <div className="text-3xl md:text-4xl font-bold text-amber">$15,900<span className="text-base font-normal text-warm-500">/year per location</span></div>
            </div>
            <Link href="https://app.forkhr.com/register" target="_blank">
              <Button size="lg" className="text-base px-6 h-11 bg-white text-warm-900 hover:bg-warm-100">
                Start saving
                <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            </Link>
        ...
        <div className="max-w-5xl mx-auto px-6 py-20 md:py-28">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {['Restaurants & cafés', 'Retail stores', 'Hotels & hospitality', 'Healthcare clinics', 'Fitness & gyms', 'Franchises'].map((industry) => (
              <div key={industry} className="flex items-center gap-2 text-sm text-warm-700 font-medium bg-white rounded-lg px-4 py-3 border border-warm-200">
